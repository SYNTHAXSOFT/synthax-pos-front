...
export class ImpuestoListarPageComponent implements OnInit {
  ...
  @Output() nuevoImpuesto  = new EventEmitter<void>();
  @Output() editarImpuesto = new EventEmitter<number>();
  ...
  get totalActivos(): number   { return this.impuestos.filter(i => i.estado === 'ACTIVO').length; }
  get totalInactivos(): number { return this.impuestos.filter(i => i.estado !== 'ACTIVO').length; }
  ...
  cargarImpuestos(): void {
    ...
    this.impuestoService.obtenerTodos().subscribe({
      next: (data) => { this.impuestos = data; this.cargando = false; },
      error: (err)  => { console.error('Error al cargar impuestos:', err); this.cargando = false; },
    });
  }

  editar(id?: number): void {
    if (!id) return;
    this.editarImpuesto.emit(id);
  }
  ...
  async desactivar(id?: number): Promise<void> {
    ...
    this.impuestoService.desactivar(id).subscribe({
      next: () => { this.toastService.success('Impuesto desactivado'); this.cargarImpuestos(); },
      error: (err) => { console.error('Error al desactivar:', err); this.toastService.error('Error al desactivar el impuesto'); },
    });
  }
}
